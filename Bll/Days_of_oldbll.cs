...
namespace Bll
{
    public class Days_of_oldbll
    {
        public RequestResult GetAllDaysOld()
        {
            ...
            {
                List<AllOld> lsts = new List<AllOld>();
                AllOld a = new AllOld();
                ...
                foreach (var item in db.days_of_old2.ToList())
                {
                    ...
                    a.d = Days_of_oldDto.DalToDto(item);
                    lsts.Add(a);
                }
        ...
        public void Update( Days_of_oldDto d)
        {
            using (dbEntities db = new dbEntities())
            {
        ...
        public void Delete(int idday)
        {
            using (dbEntities db = new dbEntities())
            {

                foreach (days_of_old2 wor in db.days_of_old2)

                {
                    if (wor.iddayo == idday)
                    {
                        db.days_of_old2.Remove(wor);
                    }
