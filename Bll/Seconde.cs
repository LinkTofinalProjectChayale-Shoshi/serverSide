...

{
    public class Seconde
    {
        ...
        public void TheSecond()
        {
            ...
            days_of_old2 dold = new days_of_old2(); 
            ...
            for (int i = 0; i < 6; i++)
            {
                ...
                for (int wor = 0; wor < numofwoincity; wor++)
                {
                    ...
                    for (int old = 0; old < numofoincity; old++)
                    {
                        ...
                        //בודק מיהם ימי העובד ע"פ הid 
                        if (wor == 0) { 
                         dold = db.days_of_old2.Where(xi => xi.conoID == co.conoID).FirstOrDefault();
                        if(dold!=null)
                ...
                temp = temp.Copy(Dictionaryaria[arraria[i]],temp);
                    ...
                    if (Dictionaryaria[arraria[i]].Rows >= (Dictionaryaria[arraria[i]].Columns) / 3 && flagp)
                    {
                        ...
                        //בודק אם היה שיבוץ -1 שמים אם לא היה שיבוץ
                        {
                            if (arrschuing[wor] != -1)
                            {
                                ...
                                if (daye == 1)
                                    daye += 30;
                                days_of_wor2 dw = db.days_of_wor2.Where(xi => xi.idwor == idwor).FirstOrDefault();
                                if(dw!=null)
                                daysw = Days_of_wor2Dto.DalToDto(dw);
                                 dold = db.days_of_old2.Where(xi => xi.idold == idold).FirstOrDefault();
                                if(dold!=null)
                                dayo = Days_of_oldDto.DalToDto(dold);
                        ...
                        while (numofwoincity < numofoincity && isplacment) { 
                            if(numofwoincity != 0)
                        {

                            Dayes day = new Dayes();List<Days_of_oldDto> dayolddto = new List<Days_of_oldDto>();
                                List<Days_of_wor2Dto> daywordto = new List<Days_of_wor2Dto>();
                                List<days_of_old2> d = db.days_of_old2.Where(c => c.Constraintsold.idcity == i + 1).ToList();
                                List<days_of_wor2> dd = db.days_of_wor2.Where(c => c.Constraintsforeigenworker.idcity == i + 1).ToList();
                                foreach (var item in d)
                                {
                                    dayolddto.Add(Days_of_oldDto.DalToDto(item));
                                }
                                foreach (var item in dd)
                                {
                                    daywordto.Add(Days_of_wor2Dto.DalToDto(item));
                                }
                                //כאן שולחים את המטיצה temp לפונקציה שעוברת על הימים ומחזירה פונקציה בהתאם לכך

                                day.RemoveDays(ref temp, ref numofwoincity, ref numofoincity, arrschuing, ref list_old, ref list_worker, countday,ref dayolddto,ref daywordto);

              
                                Dictionaryaria[arraria[i]]=Dictionaryaria[arraria[i]].CopyMatrix(temp, Dictionaryaria[arraria[i]]);
                            ...
                            for (int wor = 0; wor < numofwoincity; wor++)
                            {
                                ...
                                if (arrschuing[wor] != -1)
                                {
                                    ...
                                    Days_of_wor2Dto dw = daywordto.Where(xi => xi.idwor == idwor).FirstOrDefault();
                                    
                                    Days_of_oldDto dolds = dayolddto.Where(xi => xi.idold == idold).FirstOrDefault();
                                    
                                    for (int dayw = 0; dayw < numwork; dayw++)
                                    {
                                        if (dolds.days[dayw] && dw.days[dayw])
                                        {
